...
int count_occurence_of(const std::string& base_string, std::string pattern, size_t start, size_t end){
    ...
    return occurrences;
        ...
        inline bool filter_cue::do_with_substr(const std::string& str, size_t start, const size_t end, const char sep, size_t min_loops, const std::function<bool(const std::string&, size_t, size_t, size_t)> func){
...
//                tr = LINEAR;
            ...
            try {
                switch (channel.at(nr_channel).channel_type) {
                    case EIGHT_BIT:
                        cues.at(cue).eight_bit_frames.push_back(KeyFrame<uint8_t>(std::stoi(str.substr(start, sep - start)), tr));
                        break;
                    case SIXTEEN_BIT:
                        cues.at(cue).sixteen_bit_frames.push_back(
                                KeyFrame<uint16_t>(std::stoi(str.substr(start, sep - start)), tr));
                        break;
                    case FLOAT:
                        cues.at(cue).float_frames.push_back(KeyFrame<double>(std::stod(str.substr(start, sep - start)), tr));
                        break;
                    case COLOR:
                        cues.at(cue).color_frames.push_back(KeyFrame<dmxfish::dmx::pixel>(dmxfish::dmx::pixel(), tr));
                        const auto first_position = str.find(',', start);
                        cues.at(cue).color_frames.at(channel.at(nr_channel).index).value.hue = std::stod(str.substr(start, first_position - start));
                        const auto second_position = str.find(",", first_position + 1);
                        cues.at(cue).color_frames.at(channel.at(nr_channel).index).value.saturation = std::stod(str.substr(first_position + 1, second_position - first_position - 1));
                        cues.at(cue).color_frames.at(channel.at(nr_channel).index).value.iluminance = std::stod(str.substr(second_position + 1, end - second_position - 1));
                        break;
//                    default:
//                        return false;
                }
        ...
        bool filter_cue::handle_timestamps(size_t cue, const std::string& str, size_t start, size_t end, size_t nr_timestamp){
            ...
            return do_with_substr(str, end_ts+1, end, '&', channel.size(), std::bind(&dmxfish::filters::filter_cue::handle_frame, this, cue, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
        }
        ...
        bool filter_cue::handle_cue(const std::string& str, size_t start, size_t end, size_t cue){
            ...
            if (!do_with_substr(str, start, end_conf, '|', 1, std::bind(&dmxfish::filters::filter_cue::handle_timestamps, this, cue, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4))){
                return false;
            }
            if (end_conf < end) {
                return do_with_substr(str, end_conf + 1, end, '#', 1, std::bind(&dmxfish::filters::filter_cue::handle_cue_conf, this, cue, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
            }
            return true;
        }
...
//                    }
            ...
            double rel_time = (*time-last_timestamp)/(cues.at(active_cue).timestamps.at(frame)-last_timestamp+start_time);

            for(size_t i = 0; i < eight_bit_channels.size(); i++){
                size_t frame_index = frame*eight_bit_channels.size()+i;
                calc_transition<uint8_t>(rel_time, cues.at(active_cue).eight_bit_frames.at(eight_bit_channels.size()*active_cue + frame_index).transition, last_eight_bit_channels.at(i), cues.at(active_cue).eight_bit_frames.at(eight_bit_channels.size()*active_cue + frame_index).value, i);
            }
            for(size_t i = 0; i < sixteen_bit_channels.size(); i++){
                size_t frame_index = frame*sixteen_bit_channels.size()+i;
                calc_transition<uint16_t>(rel_time, cues.at(active_cue).sixteen_bit_frames.at(sixteen_bit_channels.size()*active_cue + frame_index).transition, last_sixteen_bit_channels.at(i), cues.at(active_cue).sixteen_bit_frames.at(sixteen_bit_channels.size()*active_cue + frame_index).value, i);
            }
            for(size_t i = 0; i < float_channels.size(); i++){
                size_t frame_index = frame*float_channels.size()+i;
                calc_transition<double>(rel_time, cues.at(active_cue).float_frames.at(float_channels.size()*active_cue + frame_index).transition, last_float_channels.at(i), cues.at(active_cue).float_frames.at(float_channels.size()*active_cue + frame_index).value, i);
            }
            for(size_t i = 0; i < color_channels.size(); i++){
                size_t frame_index = frame*color_channels.size()+i;
                calc_transition<dmxfish::dmx::pixel>(rel_time, cues.at(active_cue).color_frames.at(color_channels.size()*active_cue + frame_index).transition, last_color_channels.at(i), cues.at(active_cue).color_frames.at(color_channels.size()*active_cue + frame_index).value, i);
            }
        }


        virtual void filter_cue::setup_filter(const std::map<std::string, std::string>& configuration, const std::map<std::string, std::string>& initial_parameters, const channel_mapping& input_channels) override {
            ...
            while(true){
                ...
                if (!channel_type.compare("8bit")){
                    channel.push_back(Channel(EIGHT_BIT, eight_bit_channels.size()));
                    ...
                    channel.push_back(Channel(SIXTEEN_BIT, sixteen_bit_channels.size()));
                    ...
                    channel.push_back(Channel(FLOAT, float_channels.size()));
                    ...
                    channel.push_back(Channel(COLOR, color_channels.size()));
            ...
            if (!do_with_substr(frames, 0, frames.length(), '$', 1, std::bind(&dmxfish::filters::filter_cue::handle_cue, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4))){
                throw filter_config_exception("cue filter: unable to parse the cuelist");
            }
        }

        virtual bool filter_cue::receive_update_from_gui(const std::string& key, const std::string& _value) override {
...
//                            return true;
        ...
        virtual void filter_cue::get_output_channels(channel_mapping& map, const std::string& name) override {
            for(size_t i = 0; i<eight_bit_channels.size(); i++){
        ...
        virtual void filter_cue::update() override {
            switch (running_state) {
                case STOP: {
                    return;
                }
        ...
        virtual void filter_cue::scene_activated() override {}