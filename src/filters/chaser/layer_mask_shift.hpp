...
namespace dmxfish::filters::chaserlayers {
    ...
    class mask_shift : public chaser_layer_executor {
    private:
        cle_number_parameter np_time;
        unsigned int current_shift = 0;
        long remaining_time = 0;
        std::vector<uint16_t> mask_copy;
    public:
        mask_shift(std::list<std::string>& description, const std::map<std::string, uint16_t*>& number_inputs) {
        ...
        virtual void apply(const double elapsed_time, std::vector<dmxfish::dmx::pixel>& pixels, std::vector<uint16_t>& mask) override {
            ...
            if(this->mask_copy.size() != mask_size) {
                this->mask_copy.resize(mask_size);
            }
            ...
            for(auto i = 0; i < mask_size; i++) {
                mask[i] = mask_copy[(i + this->current_shift) % mask_copy.size()];
            }
        }
