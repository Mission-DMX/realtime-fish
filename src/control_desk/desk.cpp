...
namespace dmxfish::control_desk {
    ...
    bank::bank(std::function<void(std::string const &, bool)> rdy_handler,
               std::function<void(std::string const &, bool)> select_handler) :
            columns{}, set_ready_state_handler{std::move(rdy_handler)},
            select_state_handler{std::move(select_handler)} {
    ...
    desk::desk(std::list<std::pair<std::string, midi_device_id>> input_devices)
            : devices{}, bank_sets{}, bankset_to_index_map{} {
        ...
        for (auto &d: devices) {
            if (d->get_device_id() == midi_device_id::X_TOUCH) {
                std::array<char, 14> initial_lcd_text = {'M', 'i', 's', 's', 'i', 'o', 'n', 'V', 'e', 'r', 's', 'i',
                                                         'o', 'n'};
                ...
                std::array<char, 14> initial_lcd_text = {'E', 'x', 't', 'e', 'n', 'd', '.', 'C', 'o', 'n', 'n', 'e',
                                                         'c', 't'};
                xtouch_set_lcd_display(*d, 0, lcd_color::white_up_inverted, initial_lcd_text);
                d->schedule_transmission();
            }
        ...
        if (devices.size() == 0) {
            ...
            ::spdlog::debug("Added {} devices to control desk. Total faders: {}.", devices.size(),
                            this->max_number_of_colums);
        }
        this->gpio_event_sender = dmxfish::events::event_source::create<xtouch_gpio_event_sender>(
                get_event_storage_instance(), "Xtouch GPIO");
    }
    ...
    bool desk::set_active_bank_set(size_t index) {
        if (index < bank_sets.size() && (index != current_active_bank_set || bank_set_modification_happened)) {
            ...
            if (index == current_active_bank_set) {
                ...
                ::spdlog::error(
                        "Failed to activate bank set with index {}. There are {} registered. Remaining on index {}.",
                        index, bank_sets.size(), current_active_bank_set);
    ...
    void desk::update_fader_bank_leds() {
        ...
        for (auto d: devices) {
            xtouch_set_button_led(*d, button::BTN_FADERBANKPREV_FADERBANKPREV,
                                  index == 0 ? button_led_state::off : button_led_state::on);
            xtouch_set_button_led(*d, button::BTN_FADERBANKNEXT_FADERBANKNEXT,
                                  index + 1 == cbs.fader_banks.size() ? button_led_state::off : button_led_state::on);
            d->schedule_transmission();
        }
    }
    ...
    void desk::reset_devices() {
        ...
        for (auto d: this->devices) {
            switch (d->get_device_id()) {
                ...
                default:
                    ::spdlog::error("control desk device ID {} not yet implemented for reset.",
                                    (int) d->get_device_id());
                    break;
            }
            d->schedule_transmission();
    ...
    void desk::process_incomming_command(const midi_command &c, size_t device_index) {
        ...
        switch (d->get_device_id()) {
            ...
            case midi_device_id::X_TOUCH:
                        ...
                        if (xtouch_is_column_button(b)) {
                            ...
                            if (current_active_bank_set < bank_sets.size()) {
                                ...
                                if (cbs.active_bank < cbs.fader_banks.size()) {
                                    ...
                                    if (!col_ptr) {
                                        ::spdlog::warn("Pressed button for column {} but it is not utilized.",
                                                       column_index);
                                        return;
                                    }
                                    col_ptr->process_button_press_message(b, button_change{c.data_2});
                                    msg.set_button(missiondmx::fish::ipcmessages::ButtonCode{
                                            (unsigned int) (device_index * 256 + c.data_1)});
                                    msg.set_new_state(c.data_2 > 10 ? ::missiondmx::fish::ipcmessages::BS_BUTTON_PRESSED
                                                                    : ::missiondmx::fish::ipcmessages::BS_BUTTON_RELEASED);
                                    iom->push_msg_to_all_gui(msg,
                                                             ::missiondmx::fish::ipcmessages::MSGT_BUTTON_STATE_CHANGE);
                                }
                            }
                        ...
                        if (xtouch_is_column_fader(c.data_1)) {
                            ...
                            if (current_active_bank_set < bank_sets.size()) {
                                ...
                                if (cbs.active_bank < cbs.fader_banks.size()) {
                                    ...
                                    iom->push_msg_to_all_gui(msg,
                                                             ::missiondmx::fish::ipcmessages::MSGT_ROTARY_ENCODER_CHANGE);
                                }
                            } else {
                                ::spdlog::warn(
                                        "Received Column encoder change from {} to {} but no bank set is active.",
                                        c.data_1 - XTOUCH_ENCODER_INDEX_OFFSET, c.data_2);
                            }
                        } else {
                            if (c.data_1 == CMD_JOGWHEEL) {
                                ...
                                this->gpio_event_sender->send_message(c.data_1 == CMD_FOOTSWITCH_1 ? 0 : 1,
                                                                      c.data_2 > 0 ? 1 : 0);
                ...
                break;
            default:
                ::spdlog::error("control desk device ID {} not yet implemented for input processing.",
                                (int) d->get_device_id());
                break;
        }
        d->schedule_transmission();
    ...
    void desk::commit_readymode() {
        ...
        for (auto &column_id: bs.columns_in_ready_state) {
            if (bs.columns_map.contains(column_id)) {
                ...
                ::spdlog::error("Error in commiting desk ready state: Column id {} in ready set but not in index map.",
                                column_id);
            }
        }
    ...
    void desk::handle_bord_buttons(button b, button_change c) {
        if (b == button::BTN_GLOBALVIEW_COMMITRDY) {
        ...
        } else if (b == button::BTN_USER_OOPS && c == button_change::PRESS) {
            auto iom = get_iomanager_instance();
            iom->rollback();
            ...
            for (auto d_ptr: devices) {
                xtouch_set_button_led(*d_ptr, button::BTN_FLIP_MAINDARK,
                                      global_dark ? button_led_state::flash : button_led_state::off);
            }
        } else {
            ::missiondmx::fish::ipcmessages::button_state_change msg;
            msg.set_button(missiondmx::fish::ipcmessages::ButtonCode{(unsigned int) (b)});
            msg.set_new_state(c == button_change::PRESS ? ::missiondmx::fish::ipcmessages::BS_BUTTON_PRESSED
                                                        : ::missiondmx::fish::ipcmessages::BS_BUTTON_RELEASED);
            get_iomanager_instance()->push_msg_to_all_gui(msg,
                                                          ::missiondmx::fish::ipcmessages::MSGT_BUTTON_STATE_CHANGE);
        }
    }
    ...
    [[nodiscard]] inline bank_mode
    deduce_bank_mode(const ::missiondmx::fish::ipcmessages::fader_column &bank_definition) {
    ...
    void
    desk::update_column_from_message(const ::missiondmx::fish::ipcmessages::fader_column &msg, size_t bank_set_index) {
        ...
        col_ptr->set_display_color(lcd_color{
                (uint8_t) msg.display_color() | (msg.top_lcd_row_inverted() ? 0b00010000 : 0) |
                (msg.bottom_lcd_row_inverted() ? 0b00100000 : 0)});
    ...
    void desk::notify_showfile_changed() {
        auto iom = get_iomanager_instance();
        for (auto btn_state = iom->is_rollback_available() ? button_led_state::on
                                                           : button_led_state::off; auto &d_ptr: devices) {
            if (d_ptr->get_device_id() == midi_device_id::X_TOUCH) {
                xtouch_set_button_led(*d_ptr, button::BTN_USER_OOPS, btn_state);
                d_ptr->schedule_transmission();
            }
        }
    }
