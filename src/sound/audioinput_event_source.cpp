...
namespace dmxfish::audio {
    ...
    bool audioinput_event_source::update_conf_from_message(const missiondmx::fish::ipcmessages::event_sender &msg) {
        ...
        ::spdlog::debug("Received AudioExtract configuration command.");
        ...
        const auto new_channel_count = conf.contains("channel_count") ? std::stoi(conf["channel_count"])
                                                                      : this->channel_count;
        const auto new_sampler_rate = conf.contains("sampler_rate") ? std::stoi(conf["sampler_rate"])
                                                                    : this->sampler_rate;
        const auto new_duration = conf.contains("sample_duration") ? std::stoi(conf["sample_duration"])
                                                                   : this->record_block_duration_ms;
        ...
        if (this->use_alsa_directly) {
            ::spdlog::debug("Spawning audio extractor using ALSA API.");
    ...
    bool audioinput_event_source::common_init() {
        ...
        if (this->channel_count < 1) {
            ::spdlog::error("Beat analysis requires at least 1 input channel. {} were configured.",
                            this->channel_count);
            return false;
        }
        return true;
        ...
        while (remaining_elements_in_buffer + initial_fft_buffer_pos > ctx.fft_buffer.size()) {
            ...
            for (auto i = initial_fft_buffer_pos; i < fft_size; i++) {
                ...
                for (auto j = 0; j < buffer.cols(); j++) {
                    avg += ((double) buffer(buffer.rows() - remaining_elements_in_buffer, j)) /
                           ((double) std::numeric_limits<int>::max() / 10);
                }
            ...
            for (size_t i = 0; i < post_buffer.size(); i++) {
                ...
                post_buffer[i] = ctx.out_buffer[pos_real] * ctx.out_buffer[pos_real] +
                                 ctx.out_buffer[pos_imag] * ctx.out_buffer[pos_imag];
            }
    ...
    void audioinput_event_source::update_task_alsa() {
        ...
        if (capture_dev.prepared()) {
            ...
            ::spdlog::info("Opened sound device '{}' from '{}'. Method: ALSA", capture_dev.getDeviceName(), this->sound_dev_file);
        }
        ...
        if (const auto res = capture_dev.setChannels(this->channel_count); res < 0) {
            ::spdlog::error("Failed to set recording channel count to {}. Error: {}", this->channel_count,
                            ALSADebug().evaluateErrorS(res));
            return;
        }

        const int latency = (this->sampler_rate / 1000) * this->record_block_duration_ms;
        Eigen::Array<int, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> buffer((int) latency,
                                                                                  (int) this->channel_count);
        ...
        ::spdlog::info(
                "Successfully initialized device {} for capturing audio with format {}, {} channels, period size {} and a sampler rate of {}.",
                this->sound_dev_file, capture_dev.getFormatName(format), capture_dev.getChannels(), pSize,
                this->sampler_rate);
    ...
    void audioinput_event_source::update_task_pulse() {
        ...
        try {
            ...
            ::spdlog::info("Opened sound device for analysis. Method: Pulse, Hop Size: 512, Frame Size: {} (Recommended: 1024. Use the sample_duration parameter to adjust this.)", latency);
            ...
            while (this->running) {
                ...
                //::spdlog::debug("Min/Mav/Avg: {}/{}/{}", min(out_buf), max(out_buf), avg(out_buf));
                b.processAudioFrame(out_buf.data());
                const auto has_beat = b.beatDueInCurrentFrame();
                if (has_beat) {
                    //::spdlog::debug("Sending Event: {}:0", this->get_sender_id());
                    dmxfish::events::event e(dmxfish::events::event_type::SINGLE_TRIGGER,
