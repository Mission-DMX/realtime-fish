...
namespace dmxfish::test {
	...
	message_buffer_output::message_buffer_output(std::unique_ptr<rmrf::net::connection_client> conn_client_):
		byte_count(0),
		conn_client(move(conn_client_)),
	...
	{
		free(this->data_buff);
		::spdlog::debug("Test: destructor message buffer output");
	}
